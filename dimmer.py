...
while True:
    ...
    a = p.winfo_pointerxy()
    dimlevel=(a[1]/9)
    print(round(dimlevel))
    if dimlevel>5:
        sent="<"+ str(round(dimlevel)) + ">"
        sendToArduino(sent)
        print(sent)
    time.sleep(0.01) #This delay is for the Robin char array method. Reduced-timeout parseInt does not seem to need it.
    """
    dimlevel=(input("Enter dim level 1-99: "))
    sendToArduino("<" + str(dimlevel) + ">")"""
